...
class ImageGPTImageProcessor(BaseImageProcessor):
    ...
    def normalize(
        ...
        """
        image = rescale(image=image, scale=1 / 127.5,
    ...
    def preprocess(
        ...
        **kwargs,
        ...
        if do_color_quantize and clusters is None:
            raise ValueError(
                "Clusters must be specified if do_color_quantize is True.")
        ...
        if do_normalize:
            images = [self.normalize(
                image=image, input_data_format=input_data_format) for image in images]

        if do_color_quantize:
            images = [to_channel_dimension_format(
                image, ChannelDimension.LAST, input_data_format) for image in images]
            # color quantize from (batch_size, height, width, 3) to (batch_size, height, width)
            images = np.array(images)
            images = color_quantize(
                images, clusters).reshape(images.shape[:-1])
