...
def get_segmentation_image(
    ...
    m_id = scipy.special.softmax(masks.swapaxes(0, 1), -1)
...
class DetrImageProcessor(BaseImageProcessor):
    ...
    def post_process(self, outputs, target_sizes):
        ...
        bottom_right_x, bottom_right_y) format. Only supports MindSpore.
    ...
    def post_process_segmentation(self, outputs, target_sizes, threshold=0.9, mask_threshold=0.5):
        """
        Converts the output of [`DetrForSegmentation`] into image segmentation predictions. Only supports MindSpore.

        Args:
            outputs ([`DetrSegmentationOutput`]):
                Raw outputs of the model.
            target_sizes (`mindspore.Tensor` of shape `(batch_size, 2)` or `List[Tuple]` of length `batch_size`):
    ...
    def post_process_instance(self, results, outputs, orig_target_sizes, max_target_sizes, threshold=0.5):
        ...
        Args:
            ...
            outputs ([`DetrSegmentationOutput`]):
                Raw outputs of the model.
            orig_target_sizes (`mindspore.Tensor` of shape `(batch_size, 2)`):
                Tensor containing the size (h, w) of each image of the batch. For evaluation, this must be the original
                image size (before any data augmentation).
            max_target_sizes (`mindspore.Tensor` of shape `(batch_size, 2)`):
    ...
    def post_process_panoptic(self, outputs, processed_sizes, target_sizes=None, is_thing_map=None, threshold=0.85):
        """
        Converts the output of [`DetrForSegmentation`] into actual panoptic predictions. Only supports MindSpore.

        Args:
            outputs ([`DetrSegmentationOutput`]):
                Raw outputs of the model.
            processed_sizes (`mindspore.Tensor` of shape `(batch_size, 2)` or `List[Tuple]` of length `batch_size`):
                Torch Tensor (or list) containing the size (h, w) of each image of the batch, i.e. the size after data
                augmentation but before batching.
            target_sizes (`mindspore.Tensor` of shape `(batch_size, 2)` or `List[Tuple]` of length `batch_size`, *optional*):
            ...
            is_thing_map (`mindspore.Tensor` of shape `(batch_size, 2)`, *optional*):
        ...
        for cur_logits, cur_masks, cur_boxes, size, target_size in zip(
            ...
            def get_ids_area(masks, scores, dedup=False):
                ...
                m_id = ops.softmax(masks.swapaxes(0, 1), -1)
                ...
                np_seg_img = np.frombuffer(seg_img.tobytes())
                np_seg_img = np_seg_img.view(final_h, final_w, 3)
                m_id = mindspore.Tensor.from_numpy(rgb_to_id(np_seg_img))
    ...
    def post_process_semantic_segmentation(self, outputs, target_sizes: List[Tuple[int, int]] = None):
        """
        Converts the output of [`DetrForSegmentation`] into semantic segmentation maps. Only supports MindSpore.
    ...
    def post_process_instance_segmentation(
        ...
        """
        Converts the output of [`DetrForSegmentation`] into instance segmentation predictions. Only supports MindSpore.
