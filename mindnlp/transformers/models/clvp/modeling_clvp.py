...
def _pad_extra_bos_eos_tokens(
    ...
    if add_eos_token:
        ...
        for i, each_input_id in enumerate(input_ids):
            ...
            if np.isin(each_input_id, pad_token_id).sum():
                pos = ops.where(each_input_id == pad_token_id)[0].min()
                modified_input_ids[i] = np.concatenate(
...
class ClvpSelfAttention(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.k_proj = nn.Dense(
            self.embed_dim, self.embed_dim, has_bias=config.use_attention_bias)
    ...
    def construct(
        ...
        if past_key_value is not None:
            ...
            key_states = ops.cat((past_key, key_states), axis=-2)
            value_states = ops.cat((past_value, value_states), axis=-2)
        ...
        if rotary_pos_emb is not None:
            ...
            value_states = ops.cat((value_rot, value_pass), axis=-1)
...
class ClvpGatedLinearUnit(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.proj = nn.Dense(config.hidden_size, config.intermediate_size * 2)
...
class ClvpEncoderLayer(nn.Cell):
...
class ClvpConditioningEncoder(nn.Cell):
    ...
    def __init__(self, config: ClvpConfig):
        ...
        self.mel_attn_blocks = nn.CellList(
    ...
    def construct(
        ...
        return ops.concat([mel_spec, text_embeds], axis=1)
...
class ClvpPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, cell):
        ...
        if isinstance(cell, nn.Embedding):
            weight = np.random.normal(0.0, factor * 0.02, cell.weight.shape)
            cell.weight.set_data(mindspore.Tensor(weight, cell.weight.dtype))
        elif isinstance(cell, (nn.Dense, Conv1D, nn.Conv1d)):
            cell.weight.set_data(Initializer(Normal(factor * 0.02),
            ...
            cell.weight.set_data(Initializer(Normal(factor * (config.hidden_size**-0.5)),
                                             cell.weight.shape, cell.weight.dtype))
        elif isinstance(cell, ClvpConditioningEncoder):
            cell.mel_conv.weight.set_data(Initializer(Normal(factor),
                                                      cell.mel_conv.weight.shape, cell.mel_conv.weight.dtye))
...
class ClvpEncoder(ClvpPreTrainedModel):
    ...
    def __init__(self, config: ClvpConfig):
        ...
        self.layers = nn.CellList([ClvpEncoderLayer(config)
        ...
        self.projection = nn.Dense(
            config.hidden_size, config.projection_dim, has_bias=False)
...
class ClvpDecoder(ClvpPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.drop = nn.Dropout(p=self.config.embd_pdrop)
        self.layers = nn.CellList([ClvpDecoderLayer(self.config)
                                  for _ in range(self.config.num_hidden_layers)])
        self.layer_norm = nn.LayerNorm(
            [self.config.hidden_size], epsilon=self.config.layer_norm_epsilon)
...
class ClvpForCausalLM(ClvpPreTrainedModel):
    def __init__(self, config):
        ...
        self.lm_head = nn.Dense(self.config.hidden_size,
                                self.config.vocab_size, has_bias=True)
    ...
    def _prepare_model_inputs(
        ...
        if conditioning_embeds is not None:
            ...
            conditioning_embeds = ops.concat(
                [conditioning_embeds, mel_start_token_embedding], axis=1)
...
class ClvpModelForConditionalGeneration(ClvpPreTrainedModel):
    ...
    def __init__(self, config: ClvpConfig):
        ...
        self.logit_scale = mindspore.Parameter(
