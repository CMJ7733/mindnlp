...
class ClvpDecoderTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (ClvpModel, ClvpForCausalLM) if is_mindspore_available() else ()
...
class ClvpIntegrationTest(unittest.TestCase):
    ...
    def test_conditional_encoder(self):
        ...
        expected_shape = (self.input_features.shape[0], 18, self.model.config.decoder_config.hidden_size)
        self.assertEqual(conditioning_encoder_outputs.shape, expected_shape),
        ...
        self.assertTrue(np.allclose(conditioning_encoder_outputs[0, :3, :3], EXPECTED_OUTPUTS, atol=1e-4))
    ...
    def test_decoder_model_generate(self):
        ...
        self.assertTrue(np.allclose(autoregressive_model_output, EXPECTED_OUTPUTS))
    ...
    def test_text_and_speech_encoder_models(self):
        ...
        self.assertTrue(np.allclose(text_embeds[0, :20], EXPECTED_TEXT_EMBEDS, atol=1e-4))
        ...
        self.assertTrue(np.allclose(speech_embeds[0, :20], EXPECTED_SPEECH_EMBEDS, atol=1e-4))
    ...
    def test_full_model_integration(self):
        ...
        self.assertTrue(np.allclose(full_model_output.speech_ids.cpu()[-3:, -3:], EXPECTED_SPEECH_IDS))
        self.assertTrue(np.allclose(full_model_output.logits_per_text.cpu(), EXPECTED_SIMILARITY_SCORES))
