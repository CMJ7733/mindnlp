...
# ============================================
""" Testing suite for the Mindspore Llava-NeXT model. """
...
class LlavaNextForConditionalGenerationIntegrationTest(unittest.TestCase):
    ...
    def test_small_model_integration_test(self):
        ...
        original_input_ids = load(filepath)
        ...
        original_pixel_values = ms.load(filepath)
        assert np.allclose(original_pixel_values.asnumpy(), inputs.pixel_values.astype(ms.float16).asnumpy())
        ...
        assert np.allclose(output.logits[0, :3, :3], expected_slice, atol=1e-3)
    ...
    def test_small_model_integration_test_batch(self):
        ...
        inputs = self.processor(
            [self.prompt, self.prompt], images=[self.image, cats_image], return_tensors="ms", padding=True
        )
